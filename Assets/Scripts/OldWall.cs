...
namespace Core
{
    ...
    public class OldWall : MonoBehaviour 
	{
        ...
        private void UpdatePrefabArray() {
            ...
            for (var r = 0; r < t; r++)
            {
                if (ArrayIsEmpty(prefabArray, prefabArray.Length))
                {
                    ...
                    Debug.LogWarning("Instantiating prefabs");

                    go.transform.SetParent(transform);
                    go.name = "Instanced Wall Prefab";
            ...
            for (int i = 0; i < m_Vertices.Length; i++)
            {
                ...
                {
                    //I think check even or odd here
                    var pStart = (Rows + 1) * i;
                    var pEnd = pStart + Rows;
                    var pPoint = 1 * i;
        ...
        public void DebugEditorButton()
        {
            ArrayIsEmpty(prefabArray, BlockArray.Length);
        }
